...
def main(config, verbose):
    ...
    clust_k_to_test   = range(
                        ...
                        geodemo_config['clustering']['test']['to'])
    ...
    cgram.fit(latent_cudf) #, sample_weight=weights_cudf)
    ...
    for k in clust_k_to_test:
        ...
        kmeans = KMeans(n_clusters=k, max_iter=clust_max_iter, n_init=clust_n_init, verbose=verbose, random_state=random_seed).fit(latent_cudf) #, sample_weight=weights_cudf)
        wcss_scores.append(kmeans.inertia_)
        print('Calculating silhouette score') if verbose else None
        silh_scores.append(silhouette_score(latent_cudf, kmeans.labels_, chunksize=1000))
