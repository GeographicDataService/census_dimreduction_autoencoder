...
def main(config, verbose):
    ...
    if clust_save_clusts == "csv":
        pd.concat([ids, excl_cols, clusters_for_k_df], axis=1).to_csv(clust_output_csv, index=False)
    elif clust_save_clusts == "parquet":
        pd.concat([ids, excl_cols, clusters_for_k_df], axis=1).to_parquet(clust_output_prq, index=False)
    else:
