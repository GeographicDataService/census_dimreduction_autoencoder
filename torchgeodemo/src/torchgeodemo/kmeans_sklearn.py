...
def main(config, verbose):
    ...
    for k in clust_k_selectd:
        ...
        kmeans = KMeans(n_clusters=k, max_iter=clust_max_iter, n_init=clust_n_init, verbose=verbose, random_state=random_seed).fit(latent)
        clusters_for_k[f'{clust_nickname}_{clust_version}_k{k:03d}_now'] = kmeans.labels_
        del kmeans
    ...
    if clust_save_clusts == "csv":
        pd.concat([ids, excl_cols, excl_cols, clusters_for_k_df], axis=1).to_csv(clust_output_csv, index=False)
    elif clust_save_clusts == "parquet":
        pd.concat([ids, excl_cols, excl_cols, clusters_for_k_df], axis=1).to_parquet(clust_output_prq, index=False)
    else:
